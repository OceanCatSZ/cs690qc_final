...
class Node:
    ...
    def connect(self, neighbor, distance):
        self.neighbors[neighbor] = distance
        neighbor.neighbors[self] = distance


class Entanglement:
    def __init__(self, node1: Node, node2: Node, fidelity: float):
        self.node1 = node1
        self.node2 = node2
...
def build_uniform_chain(L_total, N_repeaters=4):
    ...
    for i in range(len(node_names) - 1):
        ...
        node1.connect(node2, distance=link_distance)

    return nodes

def generate_entanglement_BK(node1, node2, L_att=22.5, tau_attempt=1.0):
    """
